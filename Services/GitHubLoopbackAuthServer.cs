...
public sealed class GitHubLoopbackAuthServer : IDisposable
{
	...
	public GitHubLoopbackAuthServer(string callbackUrl)
	{
		...
		_expectedPath = string.IsNullOrWhiteSpace(_callbackUri.AbsolutePath) ? "/" : _callbackUri.AbsolutePath.TrimEnd('/');
	...
	public async Task<OAuthCallbackResult> WaitForCallbackAsync()
	{
		while (_listener.IsListening)
		{
			...
			{
				await WriteResponseAsync(context.Response, false, "Cette requête ne correspond pas au callback OAuth attendu.");
				continue;
			}
	...
	private static OAuthCallbackResult ParseCallback(Uri? url)
	{
		if (url is null)
		{
			return new OAuthCallbackResult(null, null, null, "Missing callback URL", null);
		}
	...
	{
		const string successHtml = "<!doctype html><html><head><meta charset=\"utf-8\"><title>GitHub login</title></head><body style=\"font-family:sans-serif;padding:24px;\"><h2>Connexion GitHub terminée</h2><p>Vous pouvez fermer cet onglet et revenir à l’application.</p></body></html>";
		var errorHtml = $"<!doctype html><html><head><meta charset=\"utf-8\"><title>GitHub login</title></head><body style=\"font-family:sans-serif;padding:24px;\"><h2>Connexion GitHub interrompue</h2><p>{WebUtility.HtmlEncode(message ?? "Vous pouvez fermer cet onglet et revenir à l’application.")}</p></body></html>";
